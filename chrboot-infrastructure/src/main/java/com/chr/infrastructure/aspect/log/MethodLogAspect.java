package com.chr.infrastructure.aspect.log;

import cn.hutool.json.JSONUtil;
import com.chr.common.jackson.JacksonUtil;
...
public class MethodLogAspect {
    ...
    @Pointcut("execution(public * com.chr..db.*Service.*(..))")
    ...
    @Pointcut("bean(*LoginUserService)")
    public void loginUserServiceLog() {
    }

    @Around("loginUserServiceLog()")
    public Object aroundLoginUserService(ProceedingJoinPoint joinPoint) throws Throwable {
        Object proceed = joinPoint.proceed();
        log.info("LOGINUSER SERVICE : {} ; REQUEST：{} ; RESPONSE : {}", joinPoint.getSignature().toShortString(),
                safeToJson(joinPoint.getArgs()), safeToJson(proceed));
        return proceed;
    }

    @AfterThrowing(value = "loginUserServiceLog()", throwing = "e")
    public void afterLoginUserServiceThrow(JoinPoint joinPoint, Exception e) {
        log.error("LOGINUSER SERVICE : {} ; REQUEST：{} ; EXCEPTION : {}", joinPoint.getSignature().toShortString(),
                safeToJson(joinPoint.getArgs()), e.getMessage());
    }
